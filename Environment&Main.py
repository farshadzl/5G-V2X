...
DQN = DeepQNetwork(epsilon=1.0 , epsilon_min = 0.01,epsilon_dec = 5e-4,gamma = 0.99, lr=0.003, mem_size = 10000, batch_size = 64, input_dims=[n_agent*n_state], n_dims=n_agent*n_state, fc1_dims=256, fc2_dims=256,fc3_dims=256, output_dims=2**n_agent)
DDPG = Agent(alpha=0.0001,beta=0.001,input_dims=n_state,tau=0.005,n_actions=n_action,gamma=0.99,max_size=1000000,C_fc1_dims=1024,C_fc2_dims=512,C_fc3_dims=256,A_fc1_dims=1024,A_fc2_dims=512,batch_size=64,n_agents=n_agent)
...
for i_episode in range(n_episode):  
    ...
    # if np.mod(i_episode,100) == 1 :
    #     print(i_episode)
    
    #     ep_rewardall = reward_mem
    #     aaa = len(ep_rewardall)
    #     mean_ep_rewardall = []
    #     w = 99
    #     for i in range(aaa - w) :
    #         temp_value = np.sum (ep_rewardall[i:-aaa+w+i])/w
    #         mean_ep_rewardall.append(temp_value)
            
    #         plt.plot(mean_ep_rewardall,label='Mean rewards DDPG algorithm')
    #         plt.xlabel("Episode")
    #         plt.ylabel("Mean Epsiodic Reward")
    #         plt.legend()
    #         plt.show()
    done = False
    ...
    for i_step in range(n_step):
        ...
        #action process-----------------------------------------------------------------
        action_DQN = DQN.choose_action(state_shape)
        action_binary = '{0:b}'.format(action_DQN) + '0' * n_agent
        action_DDPG = DDPG.choose_action(state_shape)
        action_DDPG = np.clip(action_DDPG, 0.000, 0.999)
        for i_agent in range(n_agent):
            #Allocation-------------------------------------------------------------------
            ...
            action[i_agent,0] = action_binary[i_agent] # chosen type of BS
            if action[i_agent,0] == 0 : #Allocate to Micro
                veh_BS[i_agent,i_step] = 0
                for i in range(1,n_RB): #Allocation RB
                    action[i_agent,i] = int((action_DDPG[0,i+i_agent*n_action]) * 2) 
                    veh_RB[i_agent,i,i_step] = action[i_agent,i]
                for i in range(n_RB,2*n_RB): #Allocation Power 
                    action[i_agent,i] = np.round(np.clip(action_DDPG[0,i+i_agent*n_action] * max_power_Micro, 1, max_power_Micro))  # power selected by veh
                    veh_RB_power[i_agent,i-n_RB,i_step] =  action[i_agent,i]
                    
                action[i_agent,2*n_RB] = action_DDPG[0,(2*n_RB)+(i_agent*n_action)] #Duty-cycle
                duty[i_agent] = action[i_agent,2*n_RB]
                ...
                for i in range(1,n_RB): #Allocation RB
                    ...
                    veh_RB[i_agent,i,i_step] = action[i_agent,i]
                for i in range(n_RB,2*n_RB): #Allocation Power 
                    action[i_agent,i] = np.round(np.clip(action_DDPG[0,i+i_agent*n_action] * max_power_Macro, 1, max_power_Macro))  # power selected by veh
                    veh_RB_power[i_agent,i-n_RB,i_step] =  action[i_agent,i]
                    
                action[i_agent,2*n_RB] = 1 #Duty-cycle
                duty[i_agent] = action[i_agent,2*n_RB]
        ...
        DDPG.remember(state_shape, action_DDPG, reward, new_state_shape, done)
        DQN.store_transition(state_shape, action_DQN, reward, new_state_shape)
...
#plot process-------------------------------------------------------------------------------------        
    ...
    #reward_mem.append(np.mean(reward_per_step[:]))
    for i_agent in range(n_agent):
        rate_per_episode[i_agent,i_episode] = np.mean(rate_per_step[i_agent])
        
for i_episode in range(n_episode) :
    rate_mean_all_episode[i_episode] = np.mean(rate_per_episode[:,i_episode])
    
plt.plot(i_episode_matrix,reward_per_episode) 
plt.show()
plt.plot(i_episode_matrix,rate_mean_all_episode) 
